...
const int dx=3;
const int dy=3;
//global boolean to avoid interrupting remote interrupt handler
...
//get packets from remote processor
//since this is p2 code, get the ball coordinates and remote paddle movements
//and update/redraw paddle 1 (left paddle)
void UART1IntHandler(void) {
	...
	if(systickDone==true ){//avoid interrpting the remote value interrupt
...
//initialize pb5 as an IR interrupt and enable systick
...
//pb5 interrupt handler to get IR pulse values
...
//send paddle positions via UART1 and the XBee
...
int main(void){
     ...
     * to player2. 
     */
    int positions[2];//array of 2 integers that contain positions of the paddle to send
    systickDone=true;
