 ...
 *
 * THIS IS PLAYER 2 CODE. This code receives the calculations for ball movements 
 * from remote processor and redraws ball coordinates. 
 * This code moves the right paddle locally, and sends paddle coordinates to remote processor.
 */
...
void UART1IntHandler(void) {
   ...
   // bool donePrinting=false;
    ...
    while(UARTCharsAvail(UART1_BASE))
    {
        ...
        fromUartChar2=(int *) UARTCharGet(UART1_BASE);
        if(firstChar=='p'){//if first value received is char 'p', move paddle
            fillRect(paddle1X,paddle1Y,6,20,WHITE);
            paddle1Y=fromUartChar2;
...
int main(void){
    ...
    const int PLAYER=1;//TODO: CHANGE TO 1 OR 2 DEPENDING ON PLAYER
    /*
	...
	while(1){
        ...
        if(ballXDirection==1){//movement is to the right
            ...
            //ballY=ballY+dy;
            //redraw moved ball
            drawCircle(ballX,ballY,4,BLACK);//paddle1 x,y,w,h,color
        }
        else if(ballXDirection==0){//movement is to the left
            //fill circle white
            drawCircle(ballX,ballY,4,WHITE);//paddle1 x,y,w,h,color
            //move coordinates
            ballX=ballX-dx;
            //ballY=ballY-dy;
            //redraw moved ball
        ...
        //TODO: send ball position to player 2 
        
        if (ballX-4>18 && ballX-4<24) {//hit left edge where paddle would be
            if(ballX>=paddle1X && ballX<=paddle1X+20){//if it hits within the paddle's range, bounce off
        ...
        if(ballY==4){//hits top
            ...
            //TODO: transmit 3 hex values every iteration: [ballX,ballY,remotePaddleY]
            positionArray[0]=ballX;
            positionArray[1]=ballY;
            //positionArray[2]=paddle1Y;
            SendPositions(positionArray);
            
            //SysCtlDelay(SysCtlClockGet()/3/20);//DELAY BREAKS IT!
        */
        }//systickDone end
        
		//buttonBreak increments every time the loop iterates to control
        //when to go to the next character if no button is pressed
        //TODO: dont need buttonBreak? for lab3...
        /*buttonBreak++;
        ...
        */
        ...
        if(arrayVals==32){
            ...
            switch(remoteVal){
                    ...
                    case 10111://might need to change to 10111 for voldown
                        UARTprintf("Down\n");
                        if(paddle2Y+20<128){
                        ...
                        //paddle1Y-=dy;
                        break;
                    case 1111://might need to change to 1111 for volup
                        UARTprintf("Up\n");
                        if(paddle2Y>0){
            ...
            //after buttonPressed is assigned a value, move paddle
            //if buttonPressed==1 move down, ==2 move up
        ...
        //SysCtlDelay(SysCtlClockGet()/3/8);//delay for ball movement
