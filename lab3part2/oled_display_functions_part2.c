...
//lab3 part 2-enable wireless texting via XBee module
//keep XBee on uart1, and communicate with module with 9600 baud rate
...
void configureUART1(void){
    ...
    // Initialize UART1 with 9600 baud@16mhz, 8 bit data, one stop, no parity
...
//get chars from uart1, print the character to the console and draw the character
...
void UART1IntHandler(void) {
    ...
    while(UARTCharsAvail(UART1_BASE) && donePrinting==false)
    {
		//get the char from uart1, print it to the console, and verify it is not
        //a terminating character
        fromUART=UARTCharGet(UART1_BASE);
				UARTprintf("received char=%d\n",fromUART);
        if(fromUART==0x3){//if character recieved is a terminating char, finish
            //printing
            donePrinting=true;
        }
        //draw the received character, and check for a full screen
        drawChar(remoteCharXValue,remoteCharYValue,fromUART,0,0xffff,1);
        remoteCharXValue+=7;
        if(remoteCharXValue>122){//go to new line when this line is full
            remoteCharXValue=0;
            remoteCharYValue+=7;
        }
        if(remoteCharYValue>122){//bottom half of screen full
            //clear bottom half of screen, reset char positions
            fillRect(0, SSD1351HEIGHT/2+2, SSD1351WIDTH, SSD1351HEIGHT, WHITE);
            remoteCharXValue=0;
            remoteCharYValue=128/2+5;
        }
    }
    
}

//enable the IR reciever and interrupts
...
void SendStr(void) {
    ...
    UARTIntDisable(UART1_BASE, UART_INT_TX);	// avoid critical section
    int charBufferIndexSub=0;
    while(UARTSpaceAvail(UART1_BASE) && Tx_done==false)
    {
        if (charBuffer[charBufferIndexSub]!=0x3) {
            UARTprintf("printing %c\n",charBuffer[charBufferIndexSub]);
            //UARTCharPutNonBlocking(UART1_BASE, charBuffer[charBufferIndexSub]);
						UARTCharPut(UART1_BASE, charBuffer[charBufferIndexSub]);
            charBufferIndexSub++;
            
        } else {
            //once terminating char is sent, clear the global buffer
            charBufferIndex=0;
            for(int i=0;i<200;i++){
                charBuffer[i]=0;
            }
...
int main(void){
	...
	fillScreen(0xffff);
    rawFastHLine(0,128/2,128,BLACK);
	while(1){
				
		//buttonBreak increments every time the loop iterates to control
        //when to go to the next character if no button is pressed
        buttonBreak++;
        if(buttonBreak==17500000){//go to the next character position if no button is pressed
            amountPressed=0;
            oldButtonPressed=-1;
            //charXValue+=7;
        }
        ...
        if(arrayVals==32){
            ...
            switch(remoteVal){
                ...
                case 101111:
                    UARTprintf("2\n");
                    if(oldButtonPressed==2){
                        amountPressed++;
                        toPrint=b2[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 1001111:
                    UARTprintf("3\n");
                    if(oldButtonPressed==3){
                        amountPressed++;
                        toPrint=b3[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 1110111:
                    UARTprintf("4\n");
                    if(oldButtonPressed==4){
                        amountPressed++;
                        toPrint=b4[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 110111:
                    UARTprintf("5\n");
                    if(oldButtonPressed==5){
                        amountPressed++;
                        toPrint=b5[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 1010111:
                    UARTprintf("6\n");
                    if(oldButtonPressed==6){
                        amountPressed++;
                        toPrint=b6[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 1100111:
                    UARTprintf("7\n");
                    if(oldButtonPressed==7){
                        amountPressed++;
                        toPrint=b7[amountPressed%4];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 100111:
                    UARTprintf("8\n");
                    if(oldButtonPressed==8){
                        amountPressed++;
                        toPrint=b8[amountPressed%3];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 1000111:
                    UARTprintf("9\n");
                    if(oldButtonPressed==9){
                        amountPressed++;
                        toPrint=b9[amountPressed%4];
                    }
                    else{
                        charXValue+=7;
                        charBufferIndex++;
                ...
                case 111:
                    ...
                    //drawChar(charXValue,charYValue,0xa,0,0xffff,1);//newline?
                    break;
            ...
            //after buttonPressed is assigned a value,
            if(charXValue>122){//go to new line when this line is full
                charXValue=0;
                charYValue+=7;
            }
            if(charYValue>128/2-7){//top half of screen full
                //clear top half of screen
                fillRect(0, 0, SSD1351WIDTH, SSD1351HEIGHT/2-2, WHITE);
                charXValue=0;
                charYValue=0;
            }
            oldButtonPressed=buttonPressed;
            charBuffer[charBufferIndex]=toPrint;
            
            if (charBuffer[charBufferIndex]!=0x3) {
                drawChar(charXValue,charYValue,toPrint,0,0xffff,1);
            }
            
            
            
            //when charBuffer[charBufferIndex-1]==0x3, send to remote processor
            if (charBuffer[charBufferIndex]==0x3) {
                SendStr();
            }
            //UARTCharPut(UART1_BASE, toPrint);
